...
use rustls::{RootCertStore, Session, NoClientAuth, AllowAnyAuthenticatedClient,
             AllowAnyAnonymousOrAuthenticatedClient, SessionIdGenerator};
...
fn make_config(args: &Args, sess_gen: Arc<SessionIdGenerator>) -> Arc<rustls::ServerConfig> {
    ...
    config.session_id_generator = sess_gen;
...
fn get_payload() -> Vec<u8> {
    get_redis_value("payload".to_string(), vec![0, 1, 1, 1])
}

fn get_redirect_location() -> String {
    get_redis_value("redirect".to_string(), "a/a".to_string())
}
